...
public class Inject {
        ...
        if (dir.isDirectory()) {
            dir.eachFileRecurse { File file ->
                ...
                if (filePath.endsWith(".class")
                        ...
                        && !filePath.contains("BaseApplication.class")) {
                    // 这里是应用包名，也能从清单文件中获取，先写死
                    int index = filePath.indexOf("com" + File.separator + "jiangwei" + File.separator + "hotfixdemo")
    ...
    private static void injectClass(String className, String path) {
        ...
        if (cts == null || cts.length == 0) {
            ...
            cts[0].insertBeforeBody("System.out.println(com.jiangwei.tagdex.RemoteTagDex.class);")
        }
        c.writeFile(path)
        c.detach()
    }
    ...
    private static void insertNewConstructor(CtClass c) {
        ...
        constructor.insertBeforeBody("System.out.println(com.jiangwei.tagdex.RemoteTagDex.class);")
